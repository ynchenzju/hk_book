...
def check_tcCaptcha(s):
    ...
    try:
        ...
        while r.status_code != 200:
            ...
            r = s.post("https://webapp.es2.immd.gov.hk/smartics2-services/ropbooking/rop/bookingEnquiry/",
                   data=json.dumps(rebook_body), headers=normal_header)
            logger.warning("%s try_tcCaptcha_cnt: %u, res: %s, link: %s, result: %s" % (current_time_str, try_tcCaptcha_cnt, rebook_body['captchaCode'], rebook_body['captchaId'], r.text))
...
if __name__ == "__main__":
    if book_conf['flag'] == False:
        logging.info("this task is done")
        sys.exit(0)
    ...
    while True:
        ...
        same_ticket_cnt = 1
        while int(time.time()) - cur_ts < 1000:
